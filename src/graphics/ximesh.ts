    ...
    for (let entryIdx = 0; entryIdx < entryCount; entryIdx++) {
      ...
      if (!result.placement[placementOffset]) {
        ...
        const o2w = new THREE.Matrix4(
          data.getFloat32(o2wOffset + 0, true),
          data.getFloat32(o2wOffset + 12, true),
          data.getFloat32(o2wOffset + 24, true),
          data.getFloat32(o2wOffset + 36, true),
          data.getFloat32(o2wOffset + 4, true),
          data.getFloat32(o2wOffset + 16, true),
          data.getFloat32(o2wOffset + 28, true),
          data.getFloat32(o2wOffset + 40, true),
          data.getFloat32(o2wOffset + 8, true),
          data.getFloat32(o2wOffset + 20, true),
          data.getFloat32(o2wOffset + 32, true),
          data.getFloat32(o2wOffset + 44, true),
          0,
          0,
          0,
          1,
...
/**
 * The in-game map sheet a piece of geometry belongs to, which is how a zone with floors says which
 * floor something is on. Not a height band: Garlaige's first three maps share a height range and
 * are different parts of the dungeon, while Oztroja's stack up the tower.
 */
export function mapIdOfPlacement(placement: PreparedPlacementData): number {
  return (placement.placeData >> 0x1A & 3) << 3 | placement.placeData >> 3 & 7;
}

/** The map sheet of every vertex in a built zone mesh, for filtering the thing down to one floor. */
export function mapIdPerVertex(mesh: THREE.Mesh, prep: PreparedMeshData): Uint8Array {
  const meta = mesh.geometry.getAttribute("meta");
  const out = new Uint8Array(meta.count);
  for (let v = 0; v < meta.count; v++) {
    out[v] = mapIdOfPlacement(prep.placement[meta.array[v * META_SIZE + 4]]);
  }
  return out;
}
