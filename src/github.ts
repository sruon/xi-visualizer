...
export async function propose(opts: Proposal): Promise<ProposalResult> {
  ...
  const base = opts.base || (await gh(token, `/repos/${repo}`)).default_branch;
  ...
  let existed = true;
  try {
    parentSha = (await gh(token, `/repos/${repo}/git/ref/heads/${branch}`)).object.sha;
  } catch {
    existed = false;
  }
  ...
  const openPr = async () => {
    ...
    if (open.length) return { url: open[0].html_url as string, branch, updated: true, unchanged: false };
    const pr = await gh(token, `/repos/${repo}/pulls`, {
      method: "POST",
      body: JSON.stringify({ title: opts.title, head: branch, base, body: opts.body ?? "" }),
    });
    return { url: pr.html_url as string, branch, updated: existed, unchanged: false };
  };

  // Identical tree means there is nothing to commit — don't push an empty commit.
  if (tree.sha === parent.tree.sha) {
    return existed ? { ...(await openPr()), unchanged: true } : { url: `https://github.com/${repo}`, branch, updated: false, unchanged: true };
  }
